    ...
    if (arr) {
        ...
        if (arr.indexOf(code) > -1) {
            ...
            if (arr.split('|').length >= 3) {
                alert("선호서비스센터를 3개 이상 지정 할 수 없습니다.")
                return false;
            } else {
                arr += "|" + code;
            }
...
$(window).on('load',function(){
	...
	var map_check = $('#map').html();
	console.log(map_check);
	if(map_check == ""){
		service_list();
...
function marker_click_to_list(num,lat,lng,code,rest){
	//$('#select_service').val(code);
//	console.log(num);
	//console.log(num);
	$('.info_box_wrap').hide();
    ...
    list_click_to_marker(num,lat,lng,code,rest);
...
function list_click_to_marker(num,lat,lng,code,rest){
...
function get_result_list(type,keyword,select){
	...
	result_list.html('');
	var json_url = "./../../json/service.json?ver=20210701";
...
//		$('.info_box_wrap').remove();
		...
		if(keyword == ""){
			...
			$.each(data, function(i, item){
				...
				if(item.img != ""){
					var service_img = '<div class="service_img"><img src="./../../images/service/service/service_img_'+item.img+'.jpg"></div>';
				}else {
					var service_img = "";
				}
				...
				result = '<li class="list_'+ i +' '+item.type+'" data-code="' + item.code + '">\
							<div class="list_btn" onclick="list_click_to_marker('+i+','+item.lat+','+item.lng+',\''+item.code+'\',\''+item.sat_rest+'\')">\
				...
				var content = '<div class="info_box_wrap info_box_'+type+'" id="marker_info_'+i+'" data-code="' + item.code + '">' + 
							...
							'				<li class="btn_call"><a href="'+item.site+'" target="_blank">홈페이지</a></li>' +
							'				<li class="btn_call"><a href="tel:'+item.phone+'">전화연결</a></li>' +
							'				<li class="btn_reservation"><a href="javascript:checkValidate();">선택</a></li>' +
							'            </ul>' +
							'        </div>' + 
							'    </div>' +    
							'</div>';				
				...
				kakao.maps.event.addListener(marker, 'click', function() {
					  ...
					  marker_click_to_list(i,item.lat,item.lng,item.code,item.sat_rest);
					  marker_info(i);
				});
                markers.push(marker);
