<?
    ...
    if ($_SERVER["REMOTE_ADDR"] == "1.245.137.12") {
        // echo getVehicleCouponToJson($vehicle_no);
    }
		...
		<div class="member_coupon">
			...
			<div class="coupon_top">
                <div class="input_box">
                 ...
                 <ul>
                    <li>사용 기한: <?=$coupon_sdate?> ~ <?=$coupon_edate?></li>
                    <li>주행거리: <?=$limit_mileage?></li>
                </ul> 
            </div>
            ...
            <div class="coupon_pack">
                ...
                <ul id="coupon_list" class="list">
                    ...
                    <li class="item">
                        <span><?=$row?> Km</span>
                        <ul>
                            <? foreach($coupon_row[$index] as $index2 => $row2) { 
                                if ($coupon_nm[$index2] == "1.000 Km") { $coupon_nm[$index2] = "1,000 Km"; }
                                if ($row2['use_yn'] == 'Y') { ?>
                            <li class="enable" data-date="<?=$row2['use_dt']?>" data-dealer="<?=$row2['dealer_nm']?>" data-mileage="<?=$row2['mileage']?>"><a href="#" class="used"><i class="v"></i> <?=$coupon_nm[$index2]?></a></li>
                                <? } else if ($row2['use_yn'] == 'N') { 
                                    $coupon_tot ++;
                                ?>
...
function setVehicleCoupon(_coupon) {
    coupon_km = _coupon[1]['km'].split(",");
    coupon_nm = _coupon[1]['coupon_nm'].replace("1,000 Km", "1.000 Km").split(",");
    coupon_row = _coupon[2];
    coupon_sdate = _coupon[0]['use_first_dt'].replace("-", ".");
    coupon_edate = _coupon[0]['use_end_dt'].replace("-", ".");
    coupon_tot = 0;
    ...
    $.each(coupon_km, function(index, row) {
        temp = '<li class="item">' +
                    '<span>' + row + ' Km</span>';
        temp += '<ul>';
        $.each(coupon_row[index], function(index2, row2) {
            
            if (coupon_nm[index2] == "1.000 Km") { coupon_nm[index2] = "1,000 Km"; }
            if (row2['use_yn'] == 'Y') {
                temp += '<li class="enable" data-date="' + row2['use_dt'] + '" data-dealer="' + row2['dealer_nm'] + '" data-mileage="' + row2['mileage'] + '"><a href="#" class="used"><i class="v"></i> ' + coupon_nm[index2] + '</a></li>';
            } else if (row2['use_yn'] == 'N') {
                coupon_tot ++;
