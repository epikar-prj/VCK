...
$sql = " SELECT sid, use_sms_recept_yn, use_email_recept_yn, use_push_recept_yn, use_dm_recept_yn, use_loc_service_yn FROM volvo_user WHERE master_cust_cd = '" . $master_cust_cd . "'";
...
<script>
	alert('Hej Volvo APP 런칭 이벤트가 종료 되었습니다.\n성원에 감사 드리며, 더욱 좋은 이벤트로 찾아 뵙겠습니다.');
	location.replace('/html/event/index.php');
</script>
	...
	<script>
        ...
        function checkValidate(_form) {
            ...
            if (!_form.use_sms_recept_yn.checked) {
                alert('SMS 수신 및 마케팅 활용에 대한 동의를 해주세요.');
        ...
        function updateUser(_data) {
			...
			var res;
			$.ajax({
                url: '/ajax/ajax.postEventApply.php',
                type:'post',
                data: _data,
                dataType: 'text',
