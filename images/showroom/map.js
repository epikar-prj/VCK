...
function map_list_close(){
	...
	$('.search .search_inner .search_toggle a img').attr('src','./../../images/center/toggle_list.png');
...
function map_list_show(){
	...
	$('.search .search_inner .search_toggle a img').attr('src','./../../images/center/toggle_map.png');
...
//	console.log(num);
	...
	switch(level) {
	  case 14:
		var lat_m = Number(lat) + 2.2;
		break;
	  case 13:
		var lat_m = Number(lat) + 1.1;
		break;
	  case 12:
		var lat_m = Number(lat) + .55;
		break;
	  case 11:
		var lat_m = Number(lat) + .275
		  ;
		break;
	  case 10:
		var lat_m = Number(lat) + .1375;
		break;
	  case 9:
		var lat_m = Number(lat) + .06875;
		break;
	  case 8:
		var lat_m = Number(lat) + .034375;
		break;
	  case 7:
		var lat_m = Number(lat) + .0171875;
		break;
	  case 6:
		var lat_m = Number(lat) + .00859375;
		break;
	  case 5:
		var lat_m = Number(lat) + .004296875;
		break;
	  case 4:
		var lat_m = Number(lat) + .0021484375;
		break;
	  case 3:
		var lat_m = Number(lat) + .00107421875;
		break;
	  case 2:
		var lat_m = Number(lat) + .000537109375;
		break;
	  case 1:
		var lat_m = Number(lat) + .0002685546875;
		break;
	  default:
		lat_m = Number(lat) + 1;
	}
...
//		console.log($(this).attr('class'));
		var result_info = $(this).find('.list_cont').css('display');
		
		if(result_info == 'block'){
			$(this).find('.list_cont').slideUp(function(){
			 ...
			 $(this).addClass('ov');			 
			 $(this).find('.list_cont').slideDown('200');	
		}
	});
}
...
function list_click_to_marker(num,lat,lng){
	...
	var name = $('.list_'+num+'').find('span.ti').text();
	var locPosition = new kakao.maps.LatLng(lat, lng);
	$('.info_box_wrap').hide();

	$("img[src|='https://vckiframe.com/oxp/center/images/clickmarker.png']").parent().css('margin','-42px 0px 0px -15px');
	$("img[src|='https://vckiframe.com/oxp/center/images/clickmarker.png']").parent().css('z-index','0');
	$("img[src|='https://vckiframe.com/oxp/center/images/clickmarker.png']").css('clip','rect(0px, 29px, 42px, 0px)');
	$("img[src|='https://vckiframe.com/oxp/center/images/clickmarker.png']").css('width','29px');
	$("img[src|='https://vckiframe.com/oxp/center/images/clickmarker.png']").css('height','42px');
	$("img[src|='https://vckiframe.com/oxp/center/images/clickmarker.png']").attr('src','https://vckiframe.com/oxp/center/images/marker.png');

		$("img[title|='"+name+"']").attr('src','https://vckiframe.com/oxp/center/images/clickmarker.png');	
		$("img[title|='"+name+"']").parent().css('margin','-63px 0px 0px -22px');
		$("img[title|='"+name+"']").css('clip','rect(0px, 44px, 63px, 0px)');
		$("img[title|='"+name+"']").css('width','44px');
		$("img[title|='"+name+"']").css('height','63px');
...
//		$('.info_box_wrap').remove();
		...
		if(keyword == ""){
			...
			$.each(data, function(i, item){
				...
				var time;
				if(type == 'showroom' || type == 'selekt' ){
					if(item.name == "볼보 창원 전시장" ||item.name == "볼보 광안 전시장" ||item.name == "볼보 해운대 전시장" ){
						time = '월~목 '+item.time_1+' / 금~일/공휴일 '+item.time_2+'';
					}else if(item.name == "볼보 SELEKT 수원 전시장"){
						time = '평일/토요일 '+item.time_1+' / 일요일/공휴일 '+item.time_2+'';
					}else{
						time = '평일 '+item.time_1+' / 주말/공휴일 '+item.time_2+'';
					}
				}else{
				...
				result = '<li class="list_'+ i +'" onclick="list_click_to_marker('+i+','+item.lat+','+item.lng+')" ><div class="list_btn">\
				...
				var content = '<div class="info_box_wrap info_box_'+type+'" id="marker_info_'+i+'">' + 
							...
							'				<li class="btn_reservation"><a href="/html/reservation/reservation1.php">예약하기</a></li>' +
				...
				var imageSrc = 'https://vckiframe.com/oxp/center/images/marker.png', // 마커이미지의 주소입니다    
...
//				console.log(match_keyword);
		...
		if(index == 0){
			$('ul.service').html('<li class="no_result">해당 지역에는 서비스센터가 없습니다.<br>검색어를 다시 입력하거나, 지도를 조정 해주세요.<a class="btn_reload_map" href="javascript:map_list_close();">지도 보기</a></li>');
			//mview_list_show();
			change_check = "on";
		}else{
...
/*			setTimeout(function(){
		...
		if(index == 0){
			...
			$('ul.service').html('<li class="no_result">해당 지역에는 전시장이 없습니다.<br>검색어를 다시 입력하거나, 지도를 조정 해주세요.<a class="btn_reload_map" href="javascript:map_list_close();">지도 보기</a></li>');
