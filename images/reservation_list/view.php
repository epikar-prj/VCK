...
$resvt_date = date("Y.m.d H:i", strtotime($reservation['bk_reg_dt'] . " " . $reservation['bk_reg_dt_st']));

$today = date("Y-m-d H:i");
// $today = date("Y-m-d H:i", strtotime($today . " +23 days "));
// echo "today : " . $today;
...
?>
    ...
    <div id="contents">
        ...
        <div class="container">
			<div class="info">
                ...
                <div class="btn_group">
                    <a href="/html/maintenence_near/?center=<?=$reservation['dlr_cd']?>" class="btn" id="center_around"><span>센터 인근 가볼만 한 곳</span><i><img src="/images/reservation_list/ico_right_2.png" alt="" class="ico_right_2"></i></a>
                </div>
                
            </div>
            <?
            $resvt_date2 = date("Y-m-d 23:00", strtotime(str_replace(".", "-", substr($resvt_date, 0, 10)) . " -1 days "));
            // echo $resvt_date2;

            if ($reservation['resvt_status'] != '취소') { ?>
            <div id="rvst_cancel" class="btn_group">
                <?if ($today < $resvt_date2) { ?>    
                <a href="javascript: void(0)" class="btn bg_color2" id="center_around" onclick="resrvConfirm('<?=$ro_no?>')" ><span>예약취소</span></a>
                <?} else {?>
                <a href="javascript: void(0)" class="btn bg_color2" id="center_around" onclick="alert('당일 예약 취소는 해당 센터로 연락주시기 바랍니다.')" ><span>예약취소</span></a>
                <?}?>
            </div>    
            <p class="caption">당일 취소는 신청하신 센터로 문의 해 주시기 바랍니다.</p>
            <?}?>    
	...
	<script>
        ...
        function resrvConfirm(ro_no) {
            var conf = confirm("해당 센터 예약을 취소하시겠습니까?");

            if (conf) {
                updateVehicleRepairResvtCancel(ro_no)
            }
        }
        ...
        function updateVehicleRepairResvtCancel(ro_no) {
			...
			$.ajax({
                url:'/ajax/ajax.updateRTVehicleRepairResvtCancel.php',
                type:'post',
