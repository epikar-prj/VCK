...
$query = "
	SELECT
		`sid`, `site_type`, `name`, `birth`, `hp`, `zipcode`, `addr1`, `addr2`, `email`, `car_model`, `car_num`, `buy_type`, `with_num`, `regdate`,`site_type`
	FROM
        volvo_promotion_apply
	{$SearchText}
	ORDER BY
		sid DESC
	LIMIT
...
<article class="unit-area">
	<div class="clearfix">
		...
		<table class="table4 vt-table bottom-margin">
			...
			<tbody>
				<tr>
					<th scope="col" class="vt-table-th2">보유차종</th>
		...
		<table class="table vt-table bottom-margin">
			...
			<tbody>
				<tr>
					...
					<th scope="col" class="vt-table-th">보유차종</th>
					<th scope="col" class="vt-table-th">구매형태</th>
					<th scope="col" class="vt-table-th">동반인원</th>
					<th scope="col" class="vt-table-th">신청경로</th>
				...
				<tr>
					...
					<td class="vt-table-td center"><?=$VOLVO_INFO['buy_type'][$row['buy_type']]?></td>
					<td class="vt-table-td center"><?=( trim($row['with_num']) == "Z" ) ? "X" : $row['with_num']?></td>
					<td class="vt-table-td center"><?=$OPTION_INFO['site_type'][$row['site_type']]?></td>
