...
$str_url = "?bm_sid={$bm_sid}&page={$page}&SearchCenter={$SearchCenter}&SearchCarmodel={$SearchCarmodel}&SearchSdate={$SearchSdate}&SearchEdate={$SearchEdate}&SearchTime={$SearchTime}&SearchSex={$SearchSex}&SearchColumn={$SearchColumn}&SearchValue={$SearchValue}";
...
if($listMode == "delete"){
	foreach($_POST['chk'] as $ckey => $cval){
		...
		if($sid){
			$query_chk[] = "UPDATE volvo_hejklass_{$tableNumber} SET `chkdel` = 'Y' WHERE sid = '{$sid}'";
		}
	}
